...
// IMPORTANTE: a API retorna valores em RUB por padrao; sempre forcamos currency=USD
...
async function buscarPreco(countryId, applicationId) {
  const dados = await chamarApi('get-prices', { country_id: countryId, application_id: applicationId, currency: 'USD' });
...
async function comprarNumero(countryId, applicationId, opcoes = {}) {
  ...
  if (!dados || !dados.number || !dados.request_id) {
    const erro = new Error('SMS-Man nao retornou numero valido: ' + JSON.stringify(dados));
    erro.codigo = dados && dados.error_code;
    throw erro;
  }
  return {
