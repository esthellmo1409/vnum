// Integração com a API do SMS-Man (https://sms-man.com/api)
...
async function chamarApi(acao, params = {}) {
  if (!TOKEN) {
    throw new Error('SMSMAN_API_KEY não configurado nas variáveis de ambiente');
  }
...
async function buscarPreco(countryId, applicationId) {
  const dados = await chamarApi('get-prices', { country_id: countryId });
  const precosPais = dados[countryId] || dados[String(countryId)];
  if (!precosPais) return null;

  if (applicationId !== undefined) {
    const item = precosPais[applicationId] || precosPais[String(applicationId)];
    if (!item) return null;
    return {
