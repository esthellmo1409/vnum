...
function defaultConfiguracoes() {
  return {
    ...
    ofertaNovos: {
      ativo: false,
      titulo: 'Oferta para novos clientes',
      texto: '',
      descontoPercent: 0,
      cupomCodigo: ''
    }
...
function mergeConfiguracoes(raw) {
  ...
  const oferta = Object.assign({}, base.ofertaNovos, c.ofertaNovos || {});
  oferta.ativo = !!oferta.ativo;
  oferta.titulo = String(oferta.titulo || base.ofertaNovos.titulo).slice(0, 80);
  oferta.texto = String(oferta.texto || '').slice(0, 400);
  oferta.descontoPercent = Math.max(0, Math.min(80, Number(oferta.descontoPercent) || 0));
  oferta.cupomCodigo = String(oferta.cupomCodigo || '').trim().toUpperCase().slice(0, 24);
  return {
    ...
    ofertaNovos: oferta,
    aquisicao: require('./aquisicao').mergeAquisicao(c.aquisicao)
  };
}
...
function snapshotPublico(db, extras) {
  ...
  return {
    ...
    bonus: bonus.ativo && bonus.bonusCentavos
      ...
      : { ativo: false },
    ofertaNovos: cfg.ofertaNovos && cfg.ofertaNovos.ativo ? cfg.ofertaNovos : { ativo: false },
    aquisicaoPublica: {
      indicacaoAtiva: cfg.aquisicao.indicacaoAtiva,
      comissaoAfiliadoPercent: cfg.aquisicao.comissaoAfiliadoPercent,
      comissaoPrimeiraCompraPercent: cfg.aquisicao.comissaoPrimeiraCompraPercent,
      comissaoRecorrente: cfg.aquisicao.comissaoRecorrente,
      saqueMinimoCentavos: cfg.aquisicao.saqueMinimoCentavos,
      aprovacaoObrigatoria: cfg.aquisicao.aprovacaoObrigatoria
    }
  };
}
...
function metricasConversao(db) {
  ...
  return {
    ...
    funil7d: {
      ...
      first_purchase: ev('first_purchase'),
      repeat_purchase: ev('repeat_purchase'),
      affiliate_click: ev('affiliate_click'),
      affiliate_signup: ev('affiliate_signup'),
      affiliate_purchase: ev('affiliate_purchase'),
      coupon_used: ev('coupon_used')
    }
  };
}
